...
const buildLess = async () => {
  //直接将less文件复制到打包后目录
  await cpy(`${sourceDir}/**/*.less`, targetLib + '\\components\\src')
  console.log('targetLib', targetLib)
  await cpy(`${sourceDir}/**/*.less`, targetEs + '\\components\\src')

  //获取打包后.less文件目录(lib和es一样)
  const lessFils = await glob("**/*.less", {cwd: srcDir, onlyFiles: true})

  console.log('lessFils', lessFils)
  ...
  for (let path in lessFils) {
    ...
    console.log('dirname(filePath)', dirname(filePath) )
    ...
    console.log('cssPath',cssPath, resolve(targetLib, 'components/src/' + cssPath) )
    //将css写入对应目录
    await fs.writeFile(resolve(targetLib, 'components/src/' + cssPath), code.css)
    await fs.writeFile(resolve(targetEs, 'components/src/' + '/' + cssPath), code.css)
  }
